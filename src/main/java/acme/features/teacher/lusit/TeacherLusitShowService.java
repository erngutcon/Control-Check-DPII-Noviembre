...
@Service
public class TeacherLusitShowService implements AbstractShowService<Teacher, Lusit> {

	// Internal state ---------------------------------------------------------

	@Autowired
	protected TeacherLusitRepository repository;
	...
	public boolean authorise(final Request<Lusit> request) {
		...
		blahblah = this.repository.findOneLusitById(blahblahId);
	...
	public Lusit findOne(final Request<Lusit> request) {
		...
		blahblahId = request.getModel().getInteger("id");
		result = this.repository.findOneLusitById(blahblahId);
	...
	public void unbind(final Request<Lusit> request, final Lusit entity, final Model model) {
		...
		request.unbind(entity, model, "iname", "logo", "description", "initialDate", "budget", "initDate", "finishDate", "hyperlink");
		
		final Money cost = this.convertToLocalCurrency(entity.getBudget());
